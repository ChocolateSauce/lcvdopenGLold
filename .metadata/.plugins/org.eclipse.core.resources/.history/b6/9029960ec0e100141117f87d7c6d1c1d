...
public class MainActivity extends Activity implements Drawer{
	...
	protected void onCreate(Bundle savedInstanceState) {
    	...
    	if(path.exists() && path.isDirectory()) {
    		...
    		resourceIds = new int[] {R.drawable.test};
	...
	public void onDrawFrame(GL10 gl, SpriteBatcher sb) {
	    ...
	    if (!pause) {
			if (image_num == 0) {
				sb.draw(R.drawable.test, src1, new Rect(0, 0,
						sb.getViewWidth(), sb.getViewHeight()));
				image_num++;
			} else if (image_num == 3) {
				sb.draw(R.drawable.test, src2, new Rect(0, 0,
						sb.getViewWidth(), sb.getViewHeight()));
				image_num++;
			} else {
				sb.draw(R.drawable.test, src3, new Rect(0, 0,
						sb.getViewWidth(), sb.getViewHeight()));
				if (image_num == 4)
					image_num = 0;
				else
